...
public class SVMTrain {
    ...
    public int svmTrain(boolean dividePrepared, boolean trainPrepared) {
        ...
        if (!trainPrepared && !classes.empty() && !trainingData.empty()) {
            CvSVMParams SVM_params = new SVMParams(SVM.C_SVC, SVM.RBF, 0.1, 1, 0.1, 1, 0.1, 0.1,
                    new Mat(), new CvTermCriteria().type(CV_TERMCRIT_ITER).max_iter(100000).epsilon(0.0001));
            ...
            try {
                ...
                svm.trainAuto(trainingData, classes, new Mat(), new Mat(), SVM_params, 10,
            ...
            svm.write(new FileStorage(fsTo));//todo: svm.write(fsTo, "svm");
        } else {
            try {
